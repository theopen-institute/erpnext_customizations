...
class PayrollBooking(AccountsController):
	#PG: Need to manage lifecycle controller better in the javascript; get rid of the buttons, make emps and slips
	# update automatically on changes to criteria
	def on_submit(self):
 		self.submit_salary_slips()

 	def on_update(self):
 		#PG: This should be removed; just here to make testing easier
 		#self.submit_salary_slips()
 		print("saved")
	...
	def submit_salary_slips(self):
		...
		#PG: create a warning message saying that n draft salary slips will be submitted
		for ss in ss_list:
		...
		if submitted_ss:
			...
			## PG: this make_accrual_jv_entry should be switched to general ledger entries
	...
	def enter_payroll_in_gl(self, cancel=False, adv_adj=False):
		...
		earnings = self.get_salary_component_total(component_type = "earnings") or {}
 		...
 		# PG: deal with loan details (figure out how it works)
 		# it might all be fine, but I will need some general ledger entries?
		...
		for acct in earnings:
 		...
 		print("!!!!!!!!!!!!!!!")
 		print(payable_amounts)
 		print(earnings)
 		print(against_deductions)
 		print(", ".join(list(set(against_deductions))))
 		print(against_earnings)
 		print(", ".join(list(set(against_earnings))))
			...
			for acc, amount in earnings.items():
				...
				gl_map.append(
				 	self.get_gl_dict({
						...
						#PG: add precision to "debit" (see next line "credit")
						"debit": amount,
			...
			for acc, amount in deductions.items():
				gl_map.append(
				 	self.get_gl_dict({
						...
						#PG: add precision to "credit" (see next line "debit")
						#PG: Is this specified in accounts?
						"credit": amount,
...
# 			
			if payroll_account_is_type_payable:
				#PG: for some reason, these are all getting lumped together for the payroll payable entry
...
def create_log(ss_list):
	if not ss_list:
		frappe.throw(
			_("There's no employee missing a salary slip for the given criteria. Check that Salary Slips have not already been created."),
			title='Error'
		)
	return ss_list
